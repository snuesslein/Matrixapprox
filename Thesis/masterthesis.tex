...
\chapter{Methods}\ref{chap:methods}
...
If $\Ob_{k+1} \Ob_{k+1}^\top=\eye$ the algorithm transforms $\Ob_{k}$ such that $\Ob_{k+1} \Ob_{k+1}^\top=\eye$  by calculating the SVD of $[C_k^\top A_k^\top]^\top$.
...
\subsection{Algorithm to optimize Input and Output dims}\label{subsec:optim_segmentation}
...
If the matrices are non square it might also be needed to use a different search distances for the outputs $\lsearchud$ and the input $\lsearchrl$.
...
Therefore we want to optimize in the singular values of $\tilde{H_\beta}$ and $\tilde{H_\alpha^*}$ some sense. 
...
The resulting segmentation also includes additional columns that do not correspond to input, but are already fixed.
If more than halve of these column have been grouped into the incorrectly the segmentation is flipped.
...
Because $\|A\|_\text{F}^2 = \sum_{i=1}^{m}\sum_{j=1}^{n} a_{ij}^2$ this results in the problem of partitioning the inputs and outputs such that the sum over the squared entries of $\tilde{H}_\beta$ and $\tilde{H}_\alpha^*$ is reduced.
...
The matrices $B_k$ and $F_k^*$ are replaced by their column-wise vector-2-norm $b_k^\top$ and $f_k^top$.
...
To measure the approximation error I use the operator norm of the difference between the matrix $A$ and the approximated matrix $\tilde{A}$.
...
The left Plot in Figure\,\ref{fig:example_move_bound_a} shows all the positions that are possible at every iteration.
...
First the a weight matrix from the \emph{Mobilenet V2} model is used.
The size of the matrix is $A \in \mathbb{R}^{1000 \times 1280}$.
I set the number of stages to $K = 10$ based on the approximations done in Section\,\ref{sec:Choose_K}.
In Section\,\ref{sec:Choose_K} the input and output dimensions  presumed to be constant.
This is not the case for the tested matrices.
Therefore I also tested other $K$s.
As the objective function I use the number of multiplications as defined in Equation\,\ref{eq:objective_flop}.
I chose $\epsilon = \frac{1}{4} \|A\|_H$ as the threshold value for which the segmentation is optimized.
The value of $\|A\|_H$ is calculated using the initial segmentation.
...
We can see that the number of operation decreases, as the algorithm runs.
...
\end{figure}
The resulting system requires more multiplications to compute $y=Au$ as required for a regular matrix vector multiplication.
Therefor the system complexity is reduced using balanced truncation as described in Section\,\ref{sec:approx}.
This is done for different approximation parameters $\epsilon$.
For every $\epsilon$ the number of multiplications and the approximation error $\| A-\tilde{A} \|$ is computed.
In Figure\,\ref{fig:mobilenet_err_cost} the approximation error is plotted against the number of multiplications for different approximation parameters $\epsilon$.
...
\paragraph{AlexNet}
Now a weight matrix form the \emph{AlexNet} model is represented.
This size of the matrix is $A \in \mathbb{R}^{4096 \times 9216}$.
...
Here it is important to choose an smaller $\epsilon$ for the approximation than for the adaptation.
\todo{information}
...
After moving the bounds the number of multiplications for $\epsilon = \frac{1}{4}\|A\|_\text{H}$ is reduced by \todo{calc}.
...
\end{figure}
Here in Figure\,\ref{fig:alexnet_seperation_comp} we can see that the approximation error actually increased after applying the Algorithm\,\ref{alg:optimize segemtnation}.
...
This system has the same input and output dimensions as the adapted system, but is directly calculated form the matrix $A$.
...
The number of multiplication is still $31\%$ less compared with the initial system for $\epsilon = \frac{1}{4}\|A\|_\text{H}$.
...
When the stages are split, the inputs and outputs are segmented such that a objective function is minimized.
This makes it possible to recover permuted Sequentially Semiseperable matrices.

\subsection{Illustrative example}
First the matrix is tested on a Sequential semiseperable matrix.
\todo[inline]{How detailed description?}
...
The inputs and outputs are segmented 
...
\end{figure}
During the first iteration the algorithm splits the inputs and the outputs in two groups.
After this, the stage is split into two stages.
In the second iteration the inputs and outputs of each stage are then again grouped into two subgroups each.
...
The algorithm to minimize the rank does not perform well.
Therefore the Frobenius norm is minimized.
\paragraph{Mobilenet V2}
Next the algorithm is used to represent a weight matrix from the \emph{Mobilenet V2} model.
Here the inputs and outputs are segmented such that the Forbenius norm of the Hankel matrices is minimized.
As the algorithm can only used numbers of stages that are powers of $2$, I use $K=2^3$ as it is closest to $K=10$.
The regularization parameter is set to $\gamma=9\times10^5$.
This makes sure, that the inputs and outputs are divided into two equally sized parts.
The resulting permuted matrix is shown in Figure\,\ref{fig:mobilenet_permutation}.
\begin{figure}[!htb]
	\centering
	\includegraphics[width=0.7\textwidth]{Plots/Mobilenet_permute.pdf}
...
For $\epsilon = \frac{1}{4}\|A\|_\text{H}$ the reduction of the computational cost due to the permutation is $<1\%$.
...
Not able to find sequentially semiseperable structures in weight matrices.
