...
The approximation $\breve{d}_k \propto \|H_k\|_F$ fits the behavior of the considered matrices fairly well as illustrated in Figure\,\ref{fig:approx_degree}.
\begin{figure}[htb]
	\centering
...
A change of the division between the inputs $u_k$ and $u_{k+1}$ moves the $k$-th vertical boundary left or right.
%When we want to movie the $k$-th vertical boundary left or right we change the. 
\input{diagrams/Moves/Move_I.pgf}
A change of the division between the outputs $y_k$ and $y_{k+1}$ analogously moves the $k$-th horizontal boundary up or down.
