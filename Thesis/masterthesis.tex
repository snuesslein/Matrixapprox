...
%Instead of using a matrix vector products the underlying structure is utilized.
...
%For this some structural parameters have to devised based on estimations.
...
For this we first start by considering a case where we only approximate the $k$-th stage of the causal system and the $k-1$-th state.
...
\end{equation}
is true.
Nevertheless the total error is bounded by 
\begin{equation}
	\|T-\hat{T}\| \leq \sum_{i=1}^K \|T-\hat{T}^{(k)}\| .
\end{equation}
\begin{proof}
	The proof relies on the fact that $\|T-\hat{T}^{(k)}\|$ is equal or larger than the actual error if multiple stages are approximated.
...
\begin{equation}
	\text{Error} \leq \epsilon K
\end{equation}
for the balanced truncation of a system with an approxiamtion parameter $\epsilon$ of a system with $K$ stages.
\begin{proof}
As the system is approximated with the parameter $\epsilon$, all singular values of $T-\hat{T}^{(k)}$ are smaller than $\epsilon$ for all $k \in {1,\dots,K-1}$.
\begin{equation}
\|T-\hat{T}\| \leq \sum_{i=1}^K \epsilon = K\epsilon
\end{equation}
\end{proof}
...
\end{align}
\end{subequations}
To make it possible to later split the resulting stages, the $\Sigma$s have to be removed form the matrices after splitting the stage.
...
\begin{algorithm}[htb]
	\begin{algorithmic}
		...
		\State $\tilde{D}_\beta \gets \tilde{D}[i_\text{out}:,i_\text{in}:] $		
		\\
...
This is equivalent to the squared frobenius norm as $\|H\|_\text{F}^2 = \sum \sigma_i^2$ \cite[p.~67]{bai_matrix_2021}. \todo{This was already used in this thesis. I think it is easier if i state it a second time here instead of giving a reference}
...
\end{equation}
is given.
First we decompose the error into
\begin{equation}
	T-\hat{T} =
	\begin{bmatrix}
	&\Delta_0^*\\
	...
	.
\end{equation}
The matrix $\Delta_k$ denotes the difference caused by the balanced truncation on the original system
\begin{equation}
...
C_{k[2]}\\
C_{k+1[1]}A_{k[11]}\\
\Ob_{k+2[1]}A_{k+2[11]}A_{k[11]}+
\Ob_{k+2[2]}A_{k+2[12]}A_{k[11]}
\end{bmatrix}  \R_{k[2]} u
\Bigg\|_2^2
\\
&=
...
\Big\|_2^2
\end{align}
This can be repeated until we reach $K$.
...
\begin{equation}
	...
	C_{k[2]}\\
	C_{k+1[1]}A_{k[11]}\\
	C_{k+2[1]}A_{k+1[11]}A_{k[11]}\\
	\vdots\\
	C_{K[1]}A_{K-1[11]}\dots A_{k[11]}
	\end{bmatrix}  \R_{k[2]} u
	\Bigg\|_2^2
	.
\end{equation}
...
\begin{equation}
	...
	.
\end{equation}
This proves the relation form equation 

\chapter{Random Sequentially Semiseperable matrices}\label{A:random_T}

\todo[inline]{Make ti more clear, discuss properties, explain higher ranks...}
...
\begin{equation}
	...
	\end{cases}
\end{equation}
Here the matrices $U_i \in \mathbb{R}^{p_i\times p_i}$ and $V_i \in \mathbb{R}^{m_i\times m_i}$ are random orthogonal matrices. \todo{cite paper on the paricular distribution}
