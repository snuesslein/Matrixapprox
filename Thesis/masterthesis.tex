...
When the boundaries are moved, this might result in the fact that some connections will be dropped.
For an mixed system these connections can be moved to the anticausal system.
...
When moving the boundary right the input $x_\m$ is removed form the input $u_{k+1}$ and attacehd to the input vector $u_k$.
...
Additionally it is possible to attach the vector $d_\text{new}$ to $D_k$.
\begin{figure}[htb]
	\centering
...
\todo[inline]{convert to pseudocode}
\begin{algorithm}[htb]
	\begin{algorithmic}
