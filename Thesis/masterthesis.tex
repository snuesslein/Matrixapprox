...
\renewcommand{\algorithmicensure}{\textbf{Output:\phantom{\textbf{Input}}\hspace{-0.7cm}}}
...
\todo{How to cite the Neural nets}
\begin{figure}[htb]
	\centering
...
\subsection{Algorithm to optimize Input and oputput dims}
...
\begin{algorithm}[htb]
	...
	\For{$i\gets 1$ \textbf{to} Number of iterations}
		...
		\For{$k \gets K$ \textbf{downto} $2$}
		\State \Comment{iterate over output dimensions and transform causal system to output normal and anticausal to input normal}
			\State $S_d \gets $ move down$(S)$ by $\lsearchud_i$
			\State $S_n \gets $ no move down$(S)$ 
