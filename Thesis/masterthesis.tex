...
\todo[inline]{Some text missing here}
...
This means that the the system does not have a initial ore remaining state.
...
\begin{equation}
	...
	\end{bmatrix}
\end{equation}
The mapping form the state to the output is called the Observability Matrix $\Ob$.
...
The problems depends on the basis for the states. This basis can be changed using state transformations.
...
The cost to compute a matrix vector product using a Time variing system is explained in Section\,\ref{sec:cost} and used in Section\,\ref{sec:Choose_K} to choose a number of stages. 
...
The idea is that we can recursively factor the observability matrix, as $\Ob_k$ contains both the mapping form the state $x_k$ to the output $y_k$ and the mapping from $x_k$ to the later outputs.
...
\begin{proof}
	...
	\begin{equation}\label{eq:induction_obs}
		...
		\eye
	\end{equation}
	By induction this proofs $\tilde{\Ob}_k^\top\tilde{\Ob}_k = \eye$ for all $k \in [K-1,\dots,1]$.
...
\begin{proof}
	...
	\begin{equation}
		...
		= \Sigma_k^2
	\end{equation}
	As $\Sigma_k$ is result from an SVD, the singular values are in decreasing order. 
\end{proof}
...
.
\end{equation}
The if the total number of inputs $M$ and the total number of outputs $P$ are divisible by $K$, the input and output sizes are related to the number of stages by $m = M/K$ and $p=P/K$.
This results in
\begin{align}
...
The singular values are related to the Frobenius norm by $\|H\|_F = \sqrt{\sum \sigma_i^2}$.\todo{Bronstein}
...
The number of states is not directly related to the two norm of the singular values as it depends on their distribution and the threshold value.% that depends on wanted accuracy.
...
After these changes the, mapping from the combined inputs $[u_k^\top \: u_{k+1}^\top]^\top$
to the state $x_{k+2}$ remains unchanged. Therefore the following states are still reachable and minimal.
This trivial algorithm results in new observability matrix $\tilde{\Ob}_{k+1}$ and reachability matrix $\tilde{\R}_{k+1}$ for the state $x_{k+1}$.
...
Additionally the vector $d_\text{new}$ is attached $D_k$ to represent the new connection from $u_{k+1}$ to $y_\m$.
...
\FloatBarrier
\subsection{Compute Hankel Singular Values when Moving Boundaries}\label{subsec:move_sig}
...
The algorithm derived here can move the boundaries by arbitrarily distances, as long as the other boundaries are not crossed. 
...
Then the singular values of the SVD of $\begin{bmatrix}
	C_{k+1} & d\\
	A_{k+1} & b
...
\begin{proof}
	...
	\end{equation}
	The analogous argument holds when the input dimensions are adapted
	\begin{equation}
...
As an objective function I used the number of multiplications.
Here the number of state is determined by counting the Hankel singular values bigger than a certain threshold value. 
This threshold value has to be known in advance.

If this value is not known a priori, we want to reduce the rank of the Hankel matrices.
The rank is usually not a good objective function, as the Hankelmatrices of the weight matrices often always have full rank.
Therefore we use the nuclear norm $\|H\|_*$.
The Nuclear norm is defined as the sum of the singualr values
\begin{equation}
	\|X\|_* = \sum_{i=1}^r \sigma_i
	.
\end{equation}
The norm also depends on the size of the Hankelmatrices.
As we do not want to reduce the size of the hankel matrices, we have to control for the size of the matrix.
This is done by dividing with the operator norm$\|H\|$. 
\begin{equation}\label{eq:objective_nuc}
	\f_{\text{nuc}}(H)= \frac{\|H\|_*}{\|H\|}
	.
\end{equation}
This function is also a lower bound of $\rank(H)$ as $\rank(X) \geq \|X\|_*/\|X\|$ \cite{recht_guaranteed_2010}.
...
All Hankel matrices are rank one.
...
These are scaled such that $\|U_{i[:,0]}\|_2=p_i$ and $\|V_{i[:,0]}\|_2=m_i$.
...
As search distances I used $l = [4,2,1,1]$.
...
\todo{wording: tree is not actually tree...}
