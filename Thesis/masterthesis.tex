...
\end{figure}
Here the system is structured in stages.
...
In a minimal system the state dimension is cannot be further reduced without a loos of information.
...
In \cite{dewilde_time-varying_1998} an algorithm is given for the hankel norm approxiamtion that computes a $\tilde{T}$.
...
\end{equation}
\todo[inline]{Write something on Hankel norm approx}
...
An error bound given in \cite{sandberg_balanced_2004}.
...
\end{proof}
This allows us to use algorithms that require input- or output normal forms, as systems can be both ordered and input normal $\Sigma_{\R k}=\eye$ or both ordered and output normal if $\Sigma_{\Ob k}=\eye$.
...
As ones does not want to calculate the reachability matrices directly, the algorithm employs the decomposition given in Equation\,\ref{eq:decomp_R}.
...
When using Time Varying Systems to approximate weight matrices, one is mainly interested in the cost of computing the product $y = Tu$ using time varying systems.
...
\begin{equation}
	...
	.
\end{equation}
One can also include the additions and get the cost
 \begin{equation}
...
To choose the appropriate number of stages I approximated how the cost depends on the number of stages.
...
For more reasonable values $\gamma=0.16$ the optimal number of stages is $\hat{K}=N*2^{-5}$. 
...
In Subsection\,\ref{subsec:optim_segmentation} an strategy to improve the segmentation is presented.
...
When the boundaries are moved to the left or down, some connections are no longer possible in a causal system. These will be dropped.
When moving the boundaries to the right or up new connections will be established.
...
The mapping form the input $u_\m$ to the output $y_{k+1}$ can be incorporated by adding the vector $C_{k+1}b$ to $D_{k+1}$ as a new column.
Analogously the mapping to the state $x_{k+1}$ can be incorporated by attaching the vector $A_{k+1}b$ to $B_{k+1}$.
...
If the last $\sigma$ is equal to zero, the state has to be removed. 
...
To attach the output $y_\m$ to the output vector $y_k$ the vector $c^\top B_k$ is added as last row to $D_k$ and the vector $c^\top A_k$ is added as last row to $C_k$.
...
This can be checked with the SVD of $[A_{k+1}^\top,\breve{C}_{k+1}^\top]^\top$.
...
When moving the boundary right the input $x_\m$ is removed form the input $u_{k+1}$ and attached to the input vector $\tilde{u}_k$.
...
\begin{algorithm}[htb]
	...
	\Output{System $[A,B,C,D]$ with changed input dimensions and $\R_{k} \R_{k}^\top = \eye$,\\ \spaceIO
		 Hankel singular values $\sigma$}
	%\For{$k\gets 2$ \textbf{to} $K$}
		\State Transform state $x_k$ to input normal 
		\State $b \gets B_{k}[:,:l]$
		\State $d \gets D_{k}[:,:l]$
...
When one wants obtain the optimal segmentation of a system, 
one wants to solve the optimization problem
\begin{equation}
min_\text{segmentation} f(\text{System})
\end{equation}
Where $F(\text{System})$ is some objective function.
as this is a discrete optimization problem, \todo{reword}.
...
\todo[inline]{Pseudocode? Twas umständlich aber würde vermutlich verständlcihekit verbessern...}
