...
In this section I introduce an algorithm that makes it possible to permute the rows and columns of the represented matrix, such that number of state dimensions is reduced.
The algorithm does not recover the whole permutation in one step but uses dynamic programming and recovers the permutation by solving separate subproblems.
This is done by recursively splitting the stages of the system into two stages each.
The state dimensions depend on the singular value decomposition of $H_k$.
The SVD of $H_k$ is invariant under permutation of the rows with $\Pi_R$ and permutation of the columns with $\Pi_C$ as the new decomposition
\begin{equation}
...
This uses the fact that $\Pi^\top \Pi = \eye$ for a permutation $\Pi$.
	...
	is true.
\end{proof}
This means that all permutations of rows and columns of the matrix $A$ are equivalent as long as only the inputs are only permuted inside the input vectors $u_k$ and the output vectors $y_k$.
...
The algorithm splits the stages into two stages. This is described in Subsection\,\ref{subsec:split_stage}.
...
\begin{algorithm}[htb]
	\begin{algorithmic}
		...
		\State $U,s,V^\top \gets \text{SVD}\left(\begin{bmatrix}
