...
Now we try to represent a weight matrix form the \emph{Mobilenet V2} model \cite{sandler_mobilenetv2_2019}.
The matrix is $A \in \mathbb{R}^{1000 \times 1280}$.
I choose to use some $K = 10$ based on the approximations done in Section\,\ref{sec:Choose_K}.
As the input and output dimensions are not constant as presumed in Section\,\ref{sec:Choose_K}. I tried different $K$ and adapted the segmentation using Algorithm\,\ref{alg:optimize segemtnation}.
...
The threshold value is set to $\epsilon = 0.5 \|A\|_H$, based on the initial segmentation.
\begin{figure}[!htb]
	%\centering
...
For $K=10$ we get the lowest number of multiplications.
...
\begin{figure}[!htb]
	...
	\caption{The black line indicates the number of multiplications for the regular matrix vector product. 
	}
...
This done with balanced truncation.
...
did decrease as shown in Figure\,\ref{fig:mobilenet_seperation_comp}
%\section{Setups}
