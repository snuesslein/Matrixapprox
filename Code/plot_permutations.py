...
def connection_plot(P1_inv,P2_inv,start=0,end=1,colors = None,flipxy=False,linewidth=None,ax=None):
    #cosine
    #v = np.linspace(0,1)
    ...
    for p1,p2,c in zip(P1_inv,P2_inv,colors):
        if flipxy:
            ln, =ax.plot(y*(p1-p2)+p2,x,color = c,linewidth=linewidth)
        else:
            ln, =ax.plot(x,y*(p1-p2)+p2,color = c,linewidth=linewidth)
